    ...
    let
      ...
      onAddToCart = function(event) {
        ...
        $.ajax({
          type: 'POST',
          url: '/cart/add.js',
          data: $(this).serialize(), // creates a text string in standard url encoded notation
          dataType: 'json',
          success: onCartUpdated,
          error: onError
        });
      },
      onLineRemoved = function(event) { // removes line item item. refer to line-item.liquid
        event.preventDefault();
        
        let
          $removeLink = $(this),
          removeQuery = $removeLink.attr('href').split('change?')[1];
        $.post('/cart/change.js', removeQuery, onCartUpdated, 'json'); // post(url, data, success, datatype)
      },
        ...
        $.ajax({
          ...
          success: function(context) {
            let
              ...
              dataCartHtml = $dataCartContents.html(), //turns the content inside of this element into a string that we can output anywhere else
            ...
            $miniCartContents.html(dataCartHtml); // replace the html with the datacart html 
